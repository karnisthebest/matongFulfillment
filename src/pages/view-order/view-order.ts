...
export class ViewOrderPage {
  ...
  /////////////////////////
    ...
    public GetOrderData() {
      ...
      let url = "https://localhost:44300/api/Orders/Search";// Url for get/post 
  ...
  //end Search
    ...
    public CreateCodeBox() {
      let _Subscribe: any //declaration variable to unsubscribe if any error is occurred
      let _access: boolean; // value that return to use in another function to determine whether date is complete or not
      let url = "https://localhost:44300/api/Merchandise/Search";// Url for get/post 
      let token = "this.AuthServiceProvider.token";// use to authenticate
      let body = { "IdentityId": "true" }; //the information which send to server
      if (body.IdentityId === null) { //Check data is valid or not
        return Observable.throw("Please insert body"); //throw "Please insert body"
        ...
        return Observable.create(observer => {
          _Subscribe = this.DataServiceProvider.PostData(url, body,token) //Call function PostData from data-service
            .subscribe(
              ...
              err => { console.log('Error handle', err); },//can add function to handle the action of error
              () => { console.log('GetMerchandise Done'); } // when get data is completed. It will console.log('GetMerchandise Done')
            );
          setTimeout(() => observer.next(_access), 4000); //if there is no error, return _access
          setTimeout(() => observer.complete(), 5000); // notifies the Observer that the Observable has finished if there is no error.
          setTimeout(() => observer.error(new Error('Merchandise Error!')), 10000); //throw Error
        }, err => console.error('observer : ', err))
        .subscribe(value=> {
          if(value){
            console.log("3 ",value)
